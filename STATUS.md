...
- **Multiplayer integrity (issue #4): IMPLEMENTABLE SCOPE DONE.**
  v1.11.0 — `NetHost` kick/ban (name + IP on direct/LAN, name-only on relay),
  GUI MANAGE PLAYERS panel, dedicated console + `--ban`/`--banfile`/`--record`.
  v1.12.0 — `AimAnalyzer` host-side heuristics (near-zero aim variance,
  sub-human acquisition, wrap-seam tracking), surfaced as ⚠ warnings in the
  panel + the console `watch` command, **never auto-banned**. 11 net + 11 aim
